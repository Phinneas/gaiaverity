...
```napkin
Create a flowchart:
Mushrooms appear -> Assess risk factors -> Fairy rings? -> Yes: Aerate and water deeply -> No: Check pets/children exposure -> Toxic risk? -> Yes: Remove mushrooms -> No: Monitor location -> Consistent clusters? -> Yes: Investigate buried debris -> No: No action needed
```
