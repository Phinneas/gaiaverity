...
```napkin
Create a flowchart:
Call 811 to locate utilities -> Test soil drainage -> Check local building codes -> Mark and prepare the area -> Excavate to planned depth -> Build the berm on the downhill side -> Create the inlet with river rock -> Amend the soil with sand and compost -> Install drainage if needed -> Plant and mulch -> Create overflow -> Test with a hose
```
